/**
 * Gradient fallbacks and initials for menu product images when hotlinking blocks remote URLs.
 */

/** @type {Readonly<Record<string, string>>} */
const PRODUCT_GRADIENTS = Object.freeze({
...
/** @type {Readonly<Record<string, string>>} */
const PRODUCT_TEXT_CLASSES = Object.freeze({
  e1: 'text-stone-800',
  e2: 'text-stone-900',
  e3: 'text-amber-950',
  e4: 'text-lime-950',
...
/**
 * @param {{ id?: string; category?: string; intensity?: number }} item
 * @returns {string}
 */
function getFallbackGradient(item) {
  if (item && item.id && PRODUCT_GRADIENTS[item.id]) return PRODUCT_GRADIENTS[item.id];

  if (item && item.category === 'Espresso') {
    var intensity = Number(item.intensity) || 5;
 ...
 * @returns {string}
 */
function getFallbackTextClass(item) {
  if (item && item.id && PRODUCT_TEXT_CLASSES[item.id]) return PRODUCT_TEXT_CLASSES[item.id];

  if (item && item.category === 'Espresso') {
    var intensity = Number(item.intensity) || 5;
    if (intensity >= 7) return 'text-amber-50';
    if (intensity <= 3) return 'text-stone-800';
...
/** @type {Readonly<Record<string, string>>} */
const PRODUCT_TEXT_COLORS = Object.freeze({
  e1: '#292524',
  e2: '#1c1917',
  e3: '#451a03',
  e4: '#1a2e05',
 ...
 * @returns {string}
 */
function getFallbackTextColor(item) {
  if (item && item.id && PRODUCT_TEXT_COLORS[item.id]) return PRODUCT_TEXT_COLORS[item.id];
...
const MenuImageFallbacks = Object.freeze({
  getProductInitials: getProductInitials,
  getFallbackGradient: getFallbackGradient,
  getFallbackTextClass: getFallbackTextClass,
  getFallbackTextColor: getFallbackTextColor,
});

if (typeof module !== 'undefined' && module.exports) {
  module.exports = MenuImageFallbacks;
}

if (typeof window !== 'undefined') {
  window.MenuImageFallbacks = MenuImageFallbacks;
}
